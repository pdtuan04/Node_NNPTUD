    ...
    const processReturn = async () => {
      try {
        ...
        if (!response.ok || !result?.success) {
          throw new Error(result?.message || "Xác nhận thanh toán VNPay thất bại");
        }
        ...
        navigate(`/booking/details/${bookingId}?fromPayment=1&method=VNPAY`, { replace: true });
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="container py-5 text-center">
        ...
        <p className="mt-3 mb-0">Đang xác nhận thanh toán VNPay...</p>
      </div>
    );
  }
